$PBExportHeader$c_assign_shift.sru
forward
global type c_assign_shift from s_object_display
end type
end forward
...
global c_assign_shift c_assign_shift
...
public subroutine f_set_dwo_window ();istr_dwo[1].s_dwo_default =  'd_document_assign_shift_grid'
istr_dwo[1].s_dwo_form = ''
istr_dwo[1].s_dwo_grid = 'd_document_assign_shift_grid'
...
/*
istr_dwo[2].s_dwo_default =  'd_staff_working_day_ex_form'
istr_dwo[2].s_dwo_form = 'd_staff_working_day_ex_form'
...
istr_dwo[2].s_description = 'Chi tiết phân công ca'
...
istr_dwo[2].s_dwo_details = ''
istr_dwo[2].s_dwo_master = 'd_document_assign_shift_grid;'
...
if not ib_upd_ins then
	if vdb_dept_id > 0 then
		...
		INSERT INTO staff_working_day (  ID  ,
			...
			WHERE staff.dept_id = :vdb_dept_id and staff.branch_id = :gdb_branch using it_transaction;
		...
		INSERT INTO staff_working_day (  ID  ,
			 ...
			 FROM 
					...
					join         (select :id_f_date + level - 1 f_date
							 from dual 
							 connect by level <= :ll_num_day) workday on 1=1
			WHERE  staff.branch_id = :gdb_branch using it_transaction;		
	end if
		...
		INSERT INTO staff_working_day (  ID  ,
			 ...
			 FROM 
					...
					join         (select :id_f_date + level - 1 f_date
							 from dual 
							 connect by level <= :ll_num_day) workday on 1=1
			WHERE staff.dept_id = :vdb_dept_id and staff.branch_id = :gdb_branch
		...
		INSERT INTO staff_working_day (  ID  ,
			 ...
			 FROM 
					...
					join         (select :id_f_date + level - 1 f_date
							 from dual 
							 connect by level <= :ll_num_day) workday on 1=1
			WHERE  staff.branch_id = :gdb_branch
			and not exists (select null from staff_working_day swd where swd.object_ref_id = :vdb_orders_id 
																					and swd.staff_id =  o_staff.id )
			using it_transaction;		
	end if
...
iastr_dwo_tabpage[1].str_dwo[1].s_dwo_default =  'd_staff_working_day_ex_form'
iastr_dwo_tabpage[1].str_dwo[1].s_dwo_form = ''
iastr_dwo_tabpage[1].str_dwo[1].s_dwo_grid = 'd_staff_working_day_ex_form'
...
iastr_dwo_tabpage[1].str_dwo[1].s_description ='Chi tiết phân công'
end subroutine

on c_assign_shift.create
call super::create
end on

on c_assign_shift.destroy
call super::destroy
end on
...
is_object_title = 'Phân công ca làm việc'

istr_actionpane[1].s_button_name =  'b_init_shift;b_insert;b_modify; b_filteron;b_query;b_importfile;b_refresh;b_delete;'

istr_actionpane[1].s_description = is_object_title

ids_staff_working_day = create t_ds_db
ids_staff_working_day.dataobject = 'd_staff_working_day_grid'
ids_staff_working_day.f_settransobject( SQLCA)
end event
			...
			if ldw_focus.dynamic f_get_ib_detail() then
				if ib_changed_dwo_4edit then
					if ldw_focus.dynamic f_isgrid() then
						istr_actionpane[li_idx].s_visible_buttons = 'b_init_shift;b_modify; b_filteron;b_query;b_refresh;b_delete;'
					else
						istr_actionpane[li_idx].s_visible_buttons = 'b_init_shift;b_modify; b_filteron;b_query;b_refresh;b_delete;'
					end if
				else
...
event e_window_e_importfile;call super::e_window_e_importfile;b_obj_instantiate	lb_obj_ins
t_transaction		lt_transaction

t_ds_db		lds_import,lds_staff_wd,lds_doc,lds_orders
datetime		ldt_f_date
double		ldb_staff_id,ldb_dept_id,ldb_id
string			ls_staff_code,ls_dept_name,ls_autonumber,ls_date
long			ll_rowcount,ll_idx,ll_row

lds_import = create t_ds_db
lds_import.dataobject = 'ds_import_shift_week_grid'
lb_obj_ins.is_table = 'object'

if this.f_choose_date( ) = 0 then
	gf_messagebox('m.c_assign_shift.e_window_e_import.03','Thông báo','Chưa chọn ngày phân công ca','exclamation','ok',1)
	return 0
end if

ll_rowcount = lb_obj_ins.f_get_ds_import( '', '', '','', lds_import)
if ll_rowcount > 0 then
	iw_display.f_get_transaction( lt_transaction)
	lds_staff_wd = create t_ds_db
	lds_staff_wd.dataobject = istr_dwo[4].s_dwo_default 
	lds_staff_wd.f_settransobject( lt_transaction)
	
	lds_doc = create t_ds_db
	lds_doc.dataobject = istr_dwo[1].s_dwo_default 
	lds_doc.f_settransobject( lt_transaction)
	
	lds_orders = create t_ds_db
	lds_orders.dataobject = istr_dwo[3].s_dwo_default
	lds_orders.f_settransobject( lt_transaction)
	
	
	ll_row = this.f_set_item_doc( lds_doc)
	ldb_id = this.f_create_id( )
	lds_doc.setitem( ll_row, 'ID', ldb_id)
	...
	ll_row = this.f_set_item_staff_shift( lds_staff_wd, ldb_id, lds_import,'Y')
	if ll_row > 0 then
		if lds_doc.update( true,false) = 1 then
			lds_doc.resetupdate( )
			if lds_orders.update( true,false) = 1 then
				lds_orders.resetupdate( )
				if lds_staff_wd.update( true,false) = 1 then
					gf_messagebox('m.c_assign_shift.e_window_e_import.03','Thông báo','Import thành công :' + string(ll_rowcount) + ' dòng','exclamation','ok',1)
					lds_staff_wd.resetupdate( )
		...
		gf_messagebox('m.c_assign_shift.e_window_e_import.01','Thông báo','import lịch làm việc thất bại, vui long xem lại file import hoặc liên hệ admin','exclamation','ok',1)
	end if
end if
destroy lds_import
destroy lds_doc
destroy lds_orders
destroy lds_staff_wd
iw_display.event e_refresh( )
return 0
end event

event e_window_e_postopen;call super::e_window_e_postopen;this.f_set_detail_working_day( )

return 0
end event

event e_dw_rowfocuschanged;call super::e_dw_rowfocuschanged;if rpo_dw.dataobject = istr_dwo[1].s_dwo_default or  rpo_dw.dataobject = istr_dwo[2].s_dwo_default  then
	this.f_set_detail_working_day( )
end if
return 0
end event

event e_dw_e_presave;call super::e_dw_e_presave;s_str_policy_attr	lstr_rule_attr, lstr_security_attr

if rpo_dw.dataobject = istr_dwo[2].s_dwo_default  then
	lstr_rule_attr = rpo_dw.dynamic f_get_rule_attr()
	lstr_security_attr= rpo_dw.dynamic f_get_security_attr()
	lstr_rule_attr.b_insert = false
	lstr_rule_attr.b_update = false
	lstr_rule_attr.b_delete = false
	lstr_security_attr.b_insert = false
	lstr_security_attr.b_update = false
	lstr_security_attr.b_delete = false
	rpo_dw.dynamic f_set_rule_attr( lstr_rule_attr)
	rpo_dw.dynamic f_set_security_attr( lstr_security_attr)
end if
return 0
end event

event e_dw_e_postsave;call super::e_dw_e_postsave;s_str_policy_attr	lstr_rule_attr, lstr_security_attr
t_dw_mpl	ldw_tmp

if rpo_dw.dataobject =  iastr_dwo_tabpage[1].str_dwo[1].s_dwo_default   then
	lstr_rule_attr = rpo_dw.dynamic f_get_rule_attr()
	lstr_security_attr= rpo_dw.dynamic f_get_security_attr()
	lstr_rule_attr.b_insert = true
	lstr_rule_attr.b_update = true
	lstr_security_attr.b_insert = true
	lstr_security_attr.b_update = true
	rpo_dw.dynamic f_set_rule_attr( lstr_rule_attr)
	rpo_dw.dynamic f_set_security_attr( lstr_security_attr)
	rpo_dw.dynamic f_set_editable_property( false)
	ldw_tmp = iw_display.f_get_dw( istr_dwo[1].s_dwo_default)
	if ldw_tmp.f_get_ib_editing( ) then
		iw_display.event e_save( )
		ldw_tmp.event e_save( )
		ldw_tmp.event e_resetupdate( )
	end if
elseif rpo_dw.dataobject = istr_dwo[1].s_dwo_default then
	ldw_tmp = iw_display.f_get_dw(iastr_dwo_tabpage[1].str_dwo[1].s_dwo_default )
	if ldw_tmp.f_get_ib_editing( ) then
		ldw_tmp.event e_save( )
		ldw_tmp.event e_resetupdate( )
	end if
...
event e_dw_e_itemchanged;call super::e_dw_e_itemchanged;//t_dw_mpl		ldw_swd
boolean			lb_copy_yn
double			ldb_sh_id
string				ls_staff_code,ls_dwo
long				ll_find,ll_find1,ll_idx
date				ld_trans_date

if rpo_dw.dataobject = 'd_staff_working_day_ex_form'  then
//	ldw_swd = iw_display.f_get_dw( istr_dwo[4].s_dwo_default)
	ls_staff_code = rpo_dw.getitemstring(vl_currentrow,'staff_code')
	ldb_sh_id = rpo_dw.getitemnumber(vl_currentrow,'id_tmp')
	ld_trans_date = rpo_dw.getitemdate(1,'name_'+ mid(vs_colname, lastpos(vs_colname,'_') +1 ,2))
	ls_dwo = left(rpo_dw.dataobject, len(rpo_dw.dataobject) - 5)
	if il_yesno = 0 then
		if gf_messagebox('m.c_assign_shift.e_dw_e_itemchanged.01','Thông báo','Bạn có muốn cập nhật cho tất cả các dòng cùng ngày không?','question','yesno',1) = 1 then
//			if gf_messagebox('m.c_assign_shift.e_dw_e_itemchanged.02','Thông báo','Bạn có muốn tắt thông báo cho đến khi lưu không?','question','yesno',1) = 1 then
				il_yesno = 1
//			else
//				il_yesno = 0
//			end if
...
//			end if
	...
	if ib_copy_yn then
		for ll_idx = vl_currentrow to rpo_dw.rowcount()
			ls_staff_code = rpo_dw.getitemstring(ll_idx,'staff_code')
			ll_find = ids_staff_working_day.find( " trans_date = Date('" + string(ld_trans_date) + "') and staff_code = '" + ls_staff_code +"'", ll_find +1, ids_staff_working_day.rowcount() + 1)
			if ll_find > 0 then ids_staff_working_day.setitem( ll_find, 'shift_id', ldb_sh_id)
			if ll_idx <> vl_currentrow then rpo_dw.setitem(ll_idx,vs_colname,vs_editdata)
		next
	else
		ll_find = ids_staff_working_day.find( "staff_code = '"+ ls_staff_code + "' and trans_date = Date('" + string(ld_trans_date) + "')", 1, ids_staff_working_day.rowcount())
		if ll_find > 0 then
			ids_staff_working_day.setitem( ll_find, 'shift_id', ldb_sh_id)
		end if
	end if
end if 
return 0
...
event e_dw_e_modifyrow;call super::e_dw_e_modifyrow;t_dw_mpl	ldw_tmp

if rpo_dw.dataobject =   iastr_dwo_tabpage[1].str_dwo[1].s_dwo_default  then
	ldw_tmp = iw_display.f_get_dw( istr_dwo[1].s_dwo_default)
	if not ldw_tmp.f_get_ib_editing( ) then
		ldw_tmp.event e_modifyrow( )
	end if
elseif rpo_dw.dataobject = istr_dwo[1].s_dwo_default then
	ldw_tmp = iw_display.f_get_dw(  iastr_dwo_tabpage[1].str_dwo[1].s_dwo_default )
	if not ldw_tmp.f_get_ib_editing( ) then
...
event e_dw_e_postinsertrow;call super::e_dw_e_postinsertrow;if pos(rpo_dw.dataobject,'d_document_assign_shift_grid') > 0 then
	 rpo_dw.setitem(vl_currentrow,'doc_type','PUR_INVOICE') 
	 
	rpo_dw.setitem( vl_currentrow, 'document_date',gd_session_date )
//	ls_autonumber = lb_obj_ins.f_get_autonumber( ldw_doc, 'code', rds_doc, ll_row,this)
//	rpo_dw.setitem( ll_row, 'code', ls_autonumber)
	rpo_dw.setitem( vl_currentrow, 'object_ref_table', 'ORDERS')
	rpo_dw.setitem( vl_currentrow, 'doc_type', 'ASSIGN_SHIFT')
	rpo_dw.setitem( vl_currentrow, 'status', 'new')
	rpo_dw.setitem( vl_currentrow, 'HANDLED_BY', gi_userid)	 
	 
elseif pos(rpo_dw.dataobject,'d_order_form') > 0 then
	rpo_dw.setitem( vl_currentrow, 'object_ref_table', 'DOCUMENT')
	rpo_dw.setitem( vl_currentrow, 'doc_type', 'ASSIGN_SHIFT')
...
event e_dw_e_vscroll;call super::e_dw_e_vscroll;// override //
return 0
end event

event e_dw_e_refresh;call super::e_dw_e_refresh;if rpo_dw.dataobject = istr_dwo[1].s_dwo_default then
	this.f_set_detail_working_day( )
end if
return 0
end event

event e_dw_e_predelete;call super::e_dw_e_predelete;double 		ldb_version
t_dw_mpl		ldw_working_day
if rpo_dw.dataobject = 'd_document_assign_shift_grid' then
	ldb_version = rpo_dw.getitemnumber( vl_currentrow, 'version_id')
	DELETE staff_working_day where object_ref_id = :ldb_version using it_transaction;
	if it_transaction.sqlcode <>0 then
		rollback using it_transaction;
		gf_messagebox('m.c_assogn_shift.e_dw_e_predelete.01','Thông bao','Không xoá được chi tiết phân công@'+it_transaction.sqlerrtext,'stop','ok',1)
		return -1
	end if
	ldw_working_day = iw_display.f_get_dw( 'd_staff_working_day_ex_form')
	ldw_working_day.reset( )
end if
return 0
end event
