$PBExportHeader$c_goods_delivery_misc.sru
forward
global type c_goods_delivery_misc from b_doc
end type
end forward
...
global c_goods_delivery_misc c_goods_delivery_misc
...
istr_dwo_related[3].s_text = 'In phiếu xuất kho'
...
on c_goods_delivery_misc.create
call super::create
end on

on c_goods_delivery_misc.destroy
call super::destroy
end on
...
istr_actionpane[1].sa_sub_button[2]='b_view_goods_delivery_sale;'
...
b_obj_instantiate		lbo_ins
	...
	if li_cnt > 0 then		
		...
		if li_cnt = 1 then
			select distinct f_doc_id into :ldb_f_doc_id
				from matching where t_doc_id = :ldb_doc_id using it_transaction;		
				
			select count(distinct t_doc_id) into :li_cnt
				from matching where f_doc_id = :ldb_f_doc_id using it_transaction;			
			if li_cnt = 1 then //-- 1 SO kết 1 phiếu xuát --//
				lbo_ins.f_upd_related_status( ldb_doc_id, 'new', it_transaction)
			else //-- 1 SO kết nhiều phiếu xuát --//
				lbo_ins.f_upd_related_status( ldb_doc_id, 'partial_issue', it_transaction)
			end if
		else  // nhiếu SO kết thành 1 phiếu xuất--//
			lbo_ins.f_upd_related_status( ldb_doc_id, 'new', it_transaction)
		end if
		//-- xóa --//
