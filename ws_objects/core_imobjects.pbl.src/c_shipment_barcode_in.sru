$PBExportHeader$c_shipment_barcode_in.sru
forward
global type c_shipment_barcode_in from b_doc
end type
end forward
...
global c_shipment_barcode_in c_shipment_barcode_in
...
public subroutine f_set_dwo_window ();istr_dwo[1].s_dwo_default =  'd_shipment_barcode_in_form'
istr_dwo[1].s_dwo_form = 'd_shipment_barcode_in_form'
...
istr_dwo[1].s_description = 'Quét nhập kho'
end subroutine
...
on c_shipment_barcode_in.create
call super::create
end on

on c_shipment_barcode_in.destroy
call super::destroy
end on
...
if rpo_dw.dataobject = 'd_shipment_barcode_in_form' and vs_colname = 'barcode' then
	...
	if lds_1.rowcount() = 0 then
		gf_messagebox('m.c_shipment_barcode_in.e_dw_e_itemchanged.01','Cảnh báo', 'Mã barcode sau  chưa được khai báo trong hệ thống:@'+ls_barcode,'exclamation','ok',1)
		rpo_dw.settext('')
		return 1
	end if 
	...
	if ldc_in_qty > 0 then
		gf_messagebox('m.c_shipment_barcode_in.e_dw_e_itemchanged.05','Cảnh báo', 'Mã thùng đã quét rồi','exclamation','ok',1)
//		messagebox('Cảnh báo', 'Đã quét rồi')
		rpo_dw.settext('')
		return 1
	end if
	if pos('new;planed;processing;scan in;', ls_status) > 0 then
		if  ld_shipment_date_in <> gd_session_date then
			gf_messagebox('m.c_shipment_barcode_in.e_dw_e_itemchanged.02','Cảnh báo', 'Không nhập kho vì khác ngày kế hoạch','exclamation','ok',1)
//			messagebox('Cảnh báo', 'Không nhập kho vì khác ngày kế hoạch')
			rpo_dw.settext('')
			return 1
		else
			//Lấy SL kế hoạch
			...
			ldc_trans_qty = f_cal_qty_in(ll_item_id, ll_item_spec, ls_po_nbr, ll_doc_id)
			if isnull(ldc_trans_qty) then ldc_trans_qty = 0
			//Kiểm tra nếu mã này chưa nhập kho thì cho nhập kho
			if ldc_in_qty = 0 then
				lds_1.setitem(1,'in_qty',1)
				...
				if lt_sqlca.sqlcode = 0 then
					...
					rpo_dw.setitem(vl_currentrow,'qty_in', ldc_trans_qty + 1)
					rpo_dw.setitem(vl_currentrow,'qty_remain', ldc_ctnqty - ldc_trans_qty - 1)
				end if
				if ldc_ctnqty - ldc_trans_qty - 1 <= 0 then
					gf_messagebox('m.c_shipment_barcode_in.e_dw_e_itemchanged.03','Thông báo', 'Mã hàng sau đã nhập đủ:@'+ls_item_name+'-'+ls_po_nbr,'exclamation','ok',1)
//					messagebox('Thông báo', ls_item_name + ' trên PO:' + ls_po_nbr + ' đã nhập đủ hàng')
				end if
			end if
		end if
	else//nếu KHXH đã hoàn thành
		gf_messagebox('m.c_shipment_barcode_in.e_dw_e_itemchanged.04','Thông báo', 'Kế hoạch xuất hàng đã khóa','exclamation','ok',1)
