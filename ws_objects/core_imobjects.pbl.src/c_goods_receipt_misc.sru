$PBExportHeader$c_goods_receipt_misc.sru
forward
global type c_goods_receipt_misc from b_doc
end type
end forward
...
global c_goods_receipt_misc c_goods_receipt_misc
...
public subroutine f_set_dwo_window ();istr_dwo[1].s_dwo_default =  'd_doc_goods_receipt_grid'
istr_dwo[1].s_dwo_form = 'd_doc_goods_receipt_form'
istr_dwo[1].s_dwo_grid = 'd_doc_goods_receipt_grid'
...
istr_dwo[1].s_description = 'Phiếu nhập kho theo PO'
...
istr_dwo_related[3].s_main_obj_dwo = 'd_doc_goods_receipt_grid'		//datawindow quan hệ main
istr_dwo_related[3].s_main_obj_column = 'id'			// cột quan hệ của main
istr_dwo_related[3].s_related_obj_dwo = 'dv_goods_receipt_form'	// datawindow quan hệ đến
istr_dwo_related[3].s_related_obj_column = 'id'			// cột quan hệ đến
istr_dwo_related[3].s_relatedtext_column = 'code'					
istr_dwo_related[3].s_text = 'In phiếu nhập kho'


end subroutine

public subroutine f_set_str_unit ();//--Datawindow chứa các cột cần làm tròn--//
istr_unit[1].s_dwo_handing = 'd_po_line_grid;d_po_line_grid;d_po_line_grid;d_po_line_grid;d_doc_goods_receipt_form;'
...
istr_unit[1].s_dwo_unit = 'd_doc_goods_receipt_form;d_doc_goods_receipt_form;d_doc_goods_receipt_form;d_doc_goods_receipt_form;d_doc_goods_receipt_form'
...
public subroutine f_set_str_streamvalue ();istr_streamvalue[1].s_f_obj_dwo = 'd_doc_goods_receipt_form'
istr_streamvalue[1].s_f_obj_column = 'dr_cr_object;lot_no;serial_no;'
...
istr_streamvalue[1].s_currency_dwo = 'd_doc_goods_receipt_form'
...
istr_ds_4_posting.ds_details[1].dataobject =  'd_goods_receipt_form'
istr_ds_4_posting.s_dataobject_str += 'd_goods_receipt'+'['+string(1)+']'
istr_ds_4_posting.ds_master[1] = create t_ds_db
istr_ds_4_posting.ds_master[1].dataobject = 'd_doc_inventory_receipt_form'
...
istr_ds_4_posting.ds_master[li_idx].dataobject = 'd_goods_receipt_form'	
...
on c_goods_receipt_misc.create
call super::create
end on

on c_goods_receipt_misc.destroy
call super::destroy
end on
...
is_object_title ='Phiếu nhập kho khác'
is_exrate_type = 'sell'
...
istr_actionpane[1].sa_sub_button[2]='b_view_goods_receipt;'
...
rpo_dw.setitem(vl_currentrow,'doc_type','GOODS_RECEIPT_MISC')
...
if ldb_doc_id > 0 then
	...
	if ldb_extend_id > 0 then
		...
		VALUES(:ldb_extend_id, :ldb_doc_id, 'DOCUMENT',:gi_user_comp_id, :gdb_branch, :gi_userid, sysdate, :gi_userid, sysdate,'GOODS_RECEIPT_MISC', 1,
...
b_obj_instantiate		lbo_ins
	...
	if li_cnt > 0 then
		...
		gf_messagebox('m.c_goods_receipt_misc.e_dw_e_predelete.01','Thông báo','Không xoá được phiếu đã kết với chứng từ khác, có số:@'+ls_doc_code,'stop','ok',1)
		return -1
	end if
	//-- delete matching--//
	...
	if li_cnt > 0 then
		...
		if li_cnt = 1 then
			lbo_ins.f_upd_related_status( ldb_doc_id, 'new', it_transaction)
		else
			lbo_ins.f_upd_related_status( ldb_doc_id, 'partial_issue', it_transaction)
		end if
		//-- xóa --//
...
t_dw_mpl			ldw_main
 ...
 laa_value[1] = '=GOODS_RECEIPT_MISC'
 ldw_main = iw_display.f_get_dwmain( )
