$PBExportHeader$c_goods_receipt.sru
forward
global type c_goods_receipt from b_doc
end type
end forward
...
global c_goods_receipt c_goods_receipt
...
istr_dwo_related[1].s_main_obj_dwo = 	'd_doc_goods_receipt_grid'	//datawindow quan hệ main
...
istr_dwo_related[1].s_text = 'Đơn đặt mua của : '
...
istr_ds_4_posting.ds_master[1].dataobject = 'd_doc_goods_receipt_form'
...
on c_goods_receipt.create
call super::create
end on

on c_goods_receipt.destroy
call super::destroy
end on
...
is_object_title ='Phiếu nhập kho theo PO'
is_exrate_type = 'sell'
...
rpo_dw.setitem(vl_currentrow,'doc_type','GOODS_RECEIPT')
	...
	if not this.ib_copying then
		...
		FOR li_idx = 1 to upperbound(istr_actionpane[]) 
			...
			elseif  istr_actionpane[li_idx].s_dwo_action = 'd_action_related_object' then
				istr_actionpane[li_idx].s_visible_buttons =   'b_po;'
...
if ldb_doc_id > 0 then
	...
	if ldb_extend_id > 0 then
		...
		VALUES(:ldb_extend_id, :ldb_doc_id, 'DOCUMENT',:gi_user_comp_id, :gdb_branch, :gi_userid, sysdate, :gi_userid, sysdate,'GOODS_RECEIPT', 1,
