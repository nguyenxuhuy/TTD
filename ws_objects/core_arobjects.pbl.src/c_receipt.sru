$PBExportHeader$c_receipt.sru
forward
global type c_receipt from b_doc
end type
end forward

global type c_receipt from b_doc
end type
global c_receipt c_receipt
...
istr_dwo[1].s_description = 'Phiếu thu'
...
istr_actionpane[1].sa_sub_button[2]='b_view_receipt;'
...
rpo_dw.setitem(vl_currentrow,'doc_type','RECEIPT')
...
if ldb_doc_id > 0 then
	...
	if ldb_extend_id > 0 then
		...
		VALUES(:ldb_extend_id, :ldb_doc_id, 'DOCUMENT',:gi_user_comp_id, :gdb_branch, :gi_userid, sysdate, :gi_userid, sysdate,'RECEIPT', 1,
