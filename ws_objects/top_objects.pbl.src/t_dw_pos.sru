$PBExportHeader$t_dw_pos.sru
$PBExportComments$top data window control
forward
global type t_dw_pos from datawindow
end type
end forward
...
event type long e_postinsertrow ( long vl_row )
event type integer e_preinsertrow ( )
event e_rbuttonup pbm_dwnrbuttonup
end type
global t_dw_pos t_dw_pos
...
long 						il_dragged_row, il_mouse_down_y, il_mouse_down_x
protected:

datawindow 			idw_master, iadw_detail[],  iadw_shared[], idw_total
string						is_enter_colname, is_lbttndown_colname, is_colDirlldown, is_textprev
...
public function integer f_send_message_to_object (ref datawindow rdw_handling, string vs_message, keycode v_keycode)
...
public function string f_get_autocomplete_string (string vs_colname, string vs_searchtext, string vs_dddw_ddlb, ref datawindowchild rdwc_dropdown, string vs_filter_colname)
end prototypes
...
//		li_rc = uf_save_dw_as_excel_parm(dw1, ls_pathname, lnvo_parm)
		...
		//EXCEL
		li_rc = GetFileSaveName("Select File",ls_file_name, named, "xls","Excel Files (*.xls),*.xls,")
...
event e_dwdropdown;//string 	ls_colname
...
//	else
//		this.ib_dropdowning = true
//		this.f_retrieve_dwc(ls_colname )
//	end if
//end if
end event
...
event type long e_addrow();long	ll_row
int		li_rtn

li_rtn = this.event e_preinsertrow( )

if li_rtn = -1 then return -1

ll_row = this.insertrow( 0)

this.event e_postinsertrow( ll_row)
return ll_row
end event
...
public function boolean f_isgrid ();//Return (this.describe("DataWindow.Processing") = "1")

return (right( this.dataobject, 5) = '_grid' )
end function
...
FOR li_idx = 1 to li_colCnt			
	...
	//-- build sort--//
	ls_findStr = "dwobject = '"+ls_dwo +"' AND dwcolumn = '" +las_object[li_idx]+ "' AND sort_type <> 'none' "
...
FOR li_idx = 1 to upperbound(lsa_colsort[])
	if isnull(lsa_colsort[li_idx]) then continue
...
//-- visible theo security --//
	...
	elseif  lb_visible_security and ls_criteria_dwsetup = '1' then 
		ls_modString = vs_objectname + ".Visible =1"
...
if pos(lower(ls_originalsql_nowhere), 'gutter') > 1 then
	...
	/* Không build lại các column trong dw valuset -//
...
if this.describe( "gutter_t.x") <> '?' and this.describe( "gutter_t.x") <> '!' then
	...
	this.modify( " gutter_t.width="+string(ll_len))
	this.modify( " gutter.width="+string(ll_len))
...
public function integer f_send_message_to_object (ref datawindow rdw_handling, string vs_message, keycode v_keycode);int 					li_rtn
s_object_display		lpo_handling
window				lw_display
...
if this.f_getparentwindow( lw_display) = 1 then
	lpo_handling = lw_display.dynamic f_get_obj_handling()
	if isvalid(lpo_handling) then
//		return lpo_handling.f_handle_message(rdw_handling, vs_message, v_keycode)
	end if
end if
return -1
...
//	if ll_dddw_rowcount = 0 then this.f_retrieve_dwc( vs_colname)
...
//		rdwc_dropdown.SetFilter("upper("+ ls_dddw_displaycol +") like '%"+ upper(vs_searchtext)+"%' ")
		ls_filter_str = lbo_inst.f_get_filterstring( rdwc_dropdown, vs_searchtext, vs_colname)
...
if isnull(ls_autocomplete) or ls_autocomplete = '' then
	return is_textprev
else
...
public function string f_get_autocomplete_string (string vs_colname, string vs_searchtext, string vs_dddw_ddlb, ref datawindowchild rdwc_dropdown, string vs_filter_colname);string					ls_autocomplete, ls_findexp, ls_displaydata_value, ls_dddw_displaycol, ls_dwdatatype, ls_coltype
string					ls_displaycolname, ls_filter_str
int						li_searchtextlen, li_ddlb_index
long					ll_dddw_rowcount, ll_findrow, ll_setup_row
boolean				lb_matchfound
...
//	if ll_dddw_rowcount = 0 then this.f_retrieve_dwc( vs_colname)
	...
	//-- filter --//
	IF not isnull(vs_searchtext) and trim(vs_searchtext)<>'' then 
		ls_filter_str = lbo_inst.f_get_filterstring( rdwc_dropdown, vs_searchtext, vs_filter_colname)
		rdwc_dropdown.SetFilter(ls_filter_str)
	else
		rdwc_dropdown.SetFilter("")
	end if 		
...
if this.dataobject <> '' then
	this.f_getcurrentsql( is_originalSQL_NoWhere, is_originalwhereClause)
end if
end event

event clicked;long			ll_row

If dwo.name <> 'gutter' then	//-- khi click vào gutter thì ko scroll--//
...
//		//--mở form phân quyền--//
//		if this.f_getparentwindow( lw_parent) = 1 then
////			lw_parent.f_openchildwithparm('s_w_multi_n','u_rec_security;0;' + this.dataobject)
//		end if
...
//			end if
////			lw_parent.f_openchildwithparm( 's_w_multi_n_max', lc_dwcolumn_setup)
...
IF ib_drag THEN
	this.selectrow(0, FALSE) 
	this.f_getparentwindow( lw_parent)
	lw_parent.dynamic event e_dw_dragdrop( source, source.getparent( ), row, dwo, this, parent  )
END IF
...
event itemfocuschanged;is_textprev = ''
end event

event retrieveend;
this.f_set_gutter_rowcount( )
end event
