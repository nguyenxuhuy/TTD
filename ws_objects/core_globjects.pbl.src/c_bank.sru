$PBExportHeader$c_bank.sru
forward
global type c_bank from s_object_display
end type
end forward
...
public subroutine f_set_dwo_window ();istr_dwo[1].s_dwo_default =  'd_objects_bank_grid'
istr_dwo[1].s_dwo_form = 'd_objects_bank_form'
istr_dwo[1].s_dwo_grid = 'd_objects_bank_grid'
...
istr_dwo[1].s_description = 'Ngân hàng '
...
if pos(rpo_dw.dataobject, 'd_objects_')> 0 then
	...
	rpo_dw.setitem(vl_currentrow,'object_ref_table','BANK')
...
event e_dw_e_postsave;call super::e_dw_e_postsave;double		ldb_bank_id, ldb_addr1, ldb_obj_id, ldb_curr_id
string			ls_acct_number, ls_addr, ls_swift
date			ld_dob
...
if ldb_obj_id > 0 then
	...
	//-- BANK--//
	ldb_bank_id = rpo_dw.getitemnumber(rpo_dw.getrow(), 'bank_id') 
	ls_acct_number = rpo_dw.getitemstring(rpo_dw.getrow(), 'account_number') 
	ls_swift = rpo_dw.getitemstring(rpo_dw.getrow(), 'swift_code') 
	ldb_curr_id =  rpo_dw.getitemnumber(rpo_dw.getrow(), 'currency_id') 
	if ldb_bank_id > 0 then
		//-- update--//		
		UPDATE bank set account_number = :ls_acct_number , swift_code = :ls_swift, currency_id = :ldb_curr_id
		where id = :ldb_bank_id using it_transaction;
	else
		//-- insert--//
		ldb_bank_id = lbo_ins.f_Create_id_ex( it_transaction )
		INSERT into bank (id, object_ref_id, object_ref_Table, company_id, branch_id, created_by, created_Date, last_mdf_by, last_mdf_date,
								currency_id, account_number,swift_code)
		VALUES (:ldb_bank_id, :ldb_obj_id, 'OBJECT',:gi_user_comp_id, :gdb_branch, :gi_userid, sysdate, :gi_userid, sysdate,
					:ldb_curr_id,:ls_acct_number,:ls_swift)
	...
	if ldb_addr1 > 0 then
		...
		INSERT into business_address (id, object_ref_id, object_ref_Table, created_by, created_Date, last_mdf_by, last_mdf_date, address)
		VALUES (:ldb_addr1, :ldb_bank_id, 'BANK', :gi_userid, sysdate, :gi_userid, sysdate,:ls_addr)
		using it_transaction;
	end if		
...
event e_dw_e_predelete;call super::e_dw_e_predelete;double		ldb_bank_id


	//-- VENDOR--//
	ldb_bank_id = rpo_dw.getitemnumber(vl_currentrow, 'bank_id') 
...
//	end if
	if ldb_bank_id > 0 then
		...
		DELETE  bank where id = :ldb_bank_id using it_transaction;
		//--ORG--//	
//		DELETE organization where object_ref_id = :ldb_vender_id using it_transaction;	
		//--BUSINESS_ADDRESS--//
		DELETE business_address 
		where object_ref_id = :ldb_bank_id using it_transaction;	
		//--POEPLE--//			
//		DELETE people 
