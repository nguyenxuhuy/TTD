$PBExportHeader$ur_timesheet.sru
forward
global type ur_timesheet from b_report
end type
end forward

global type ur_timesheet from b_report
end type
global ur_timesheet ur_timesheet
...
iastr_dwo_tabpage[1].str_dwo[1].s_dwo_default = 'dr_timesheet_shift_aia_form'
iastr_dwo_tabpage[1].str_dwo[1].s_dwo_form = 'dr_timesheet_shift_aia_form'
...
//	
	...
	ldb_workhour = lds_staff_working_day.getitemnumber( li_idx, 'work_hour')
	if isnull(ldb_workhour) then ldb_workhour = 0
	...
	if ldb_workhour > 0 then
		ldw_report.setitem( li_ins_row, 'sh_code_' + string(ll_idx1), lds_staff_working_day.getitemstring( li_idx, 'shift_code'))
	end if
...
on ur_timesheet.create
call super::create
end on

on ur_timesheet.destroy
call super::destroy
end on
...
istr_report.s_dwo_report = 'dr_timesheet_gopak_form'
