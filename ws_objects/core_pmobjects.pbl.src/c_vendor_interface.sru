$PBExportHeader$c_vendor_interface.sru
forward
global type c_vendor_interface from b_interface
end type
end forward
...
global c_vendor_interface c_vendor_interface
...
if lds_interface.update(true, false ) = 1 then
	...
	gf_messagebox('m.c_vendor_interface.e_action_load_file.01','Thông báo','Hoàn thành nạp dữ liệu','information','ok',1)
else
	gf_messagebox('m.c_vendor_interface.e_action_load_file.01','Thông báo','Lỗi nạp dữ liệu :@'+ lt_transaction.sqlerrtext ,'exclamation','ok',1)
...
long					ll_row
string					ls_duplicated_colnames,ls_sort, ls_colname
...
if ldw_interface.dataobject = 'd_vendor_interface_grid' then	
...
if ldw_interface.dataobject = 'd_vendor_interface_grid' then	
	...
	laa_arg[] = laa_empty[]
	laa_arg[1] = '=u_valueset_entry@vendpost_grp'
...
//  HANDLED_BY        ,
	...
	laa_arg[2] = '=Y'
	laa_arg[3] = '=STAFF'	
	lds_chk_key.f_add_where( 'company_id;active_yn;object_ref_table;', laa_arg[])
	lds_chk_key.f_settransobject( sqlca)
	lds_chk_key.retrieve( )
	if this.f_chk_interface_foreignkey( ldw_interface,'handled_by' , lds_chk_key, 'NAME') = -1 then return -1	
...
gf_messagebox('m.c_vendor_interface.e_action_check.01','Thông báo','Kiểm tra hoàn thành','information','ok',1)
...
long							ll_rowcnt, ll_row_import
string							las_value_detail1[],las_value_detail2[],las_value_detail3[],ls_value_master
...
if ldw_interface.dataobject = 'd_vendor_interface_grid' then
	...
	DO while ll_row_import > 0
		...
		lstr_detail1[1].s_table = 'VENDOR'
		lstr_detail1[1].s_t_colnames = 'company_id,branch_id,created_by, created_date,last_mdf_by, last_mdf_date,CUSTOMER_YN,PAYMENT_TERM,DEFAULT_CURRENCY,'+&
												'DELIVERY_MODE,PAYMENT_METHOD,COUNTRY'
		lstr_detail1[1].s_dwi_colnames = 'company_id,branch_id,created_by, created_date,last_mdf_by, last_mdf_date,CUSTOMER_YN,PAYMENT_TERM,DEFAULT_CURRENCY,'+&
												'DELIVERY_MODE,PAYMENT_METHOD,COUNTRY'
		lstr_detail1[1].s_fk_colnames =  'PAYMENT_TERM,DEFAULT_CURRENCY,DELIVERY_MODE,PAYMENT_METHOD,COUNTRY'
		lstr_detail1[1].s_pk_colnames =  'code,code,code,code,code'
		...
		laa_arg[1] = '=u_valueset_entry@country'
...
istr_dwo[1].s_dwo_default = 'd_vendor_interface_parm_form' 
istr_dwo[1].s_dwo_form = 'd_vendor_interface_parm_form'
...
istr_dwo[2].s_dwo_default = 'd_vendor_interface_grid' 
istr_dwo[2].s_dwo_form = ''
istr_dwo[2].s_dwo_grid = 'd_vendor_interface_grid'
...
on c_vendor_interface.create
call super::create
end on

on c_vendor_interface.destroy
call super::destroy
end on
...
t_dw_mpl			ldw_parm, ldw_vendor



ldw_parm = iw_display.f_get_dw('d_vendor_interface_parm_form')
ldw_parm.insertrow( 0)
ldw_vendor =  this.iw_display.dynamic f_get_dw( 2)
this.iw_display.f_set_idwfocus(ldw_vendor)

this.f_ctrl_actionbuttons(ldw_vendor)
...
if rpo_dw.dataobject = 'd_vendor_interface_parm_form' then
	...
	if ldw_item.f_get_ib_editing( ) then
		...
		gf_messagebox('m.c_vendor_interface.e_dw_e_itemchanged.01','Thông báo','Phải lưu trước khi thay đổi bảng nạp dữ liệu','exclamation','ok',1)
...
if rdw_handling.dataobject = 'd_vendor_interface_parm_form' then 
	return -9
end if
return ancestorReturnValue
end event
