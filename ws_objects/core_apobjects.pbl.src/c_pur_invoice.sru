$PBExportHeader$c_pur_invoice.sru
forward
global type c_pur_invoice from b_doc
end type
end forward

global type c_pur_invoice from b_doc
end type
global c_pur_invoice c_pur_invoice
...
public subroutine f_set_dwo_window ();istr_dwo[1].s_dwo_default =  'd_document_ap_grid'
istr_dwo[1].s_dwo_form = 'd_document_ap_form'
istr_dwo[1].s_dwo_grid = 'd_document_ap_grid'
...
istr_dwo[1].s_description = 'Hóa đơn mua'
...
istr_unit[1].s_dwo_handing = 'd_pur_invoice_line_grid;d_pur_invoice_line_grid;d_pur_invoice_line_grid;d_pur_invoice_line_grid;d_document_ap_form;'
...
istr_unit[1].s_dwo_unit = 'd_pur_invoice_line_grid;d_document_ap_form;d_document_ap_form;d_document_ap_form;d_document_ap_form;'
...
istr_dwo_related[3].s_main_obj_dwo_copy[1] = 'd_document_ap_grid' // datawindow copy từ
...
istr_dwo_related[3].s_main_obj_dwo_copy[2] = 'd_document_ap_form'
...
istr_dwo_related[3].s_match_f_dwo[2] = 'd_document_ap_form'
...
istr_dwo_related[4].s_related_obj_name = 'u_pur_invoice'					// đối tượng liên quan
...
istr_dwo_related[4].s_text = 'Nhân bản từ hóa đơn mua:'
...
istr_ds_4_posting.ds_details[1].dataobject =  'd_invoice_pur_form'
istr_ds_4_posting.s_dataobject_str += 'd_invoice_pur'+'['+string(1)+']'
istr_ds_4_posting.ds_master[1] = create t_ds_db
istr_ds_4_posting.ds_master[1].dataobject = 'd_document_ap_form'
...
istr_ds_4_posting.ds_details[li_idx].dataobject =  'd_pur_invoice_line_grid'
istr_ds_4_posting.s_dataobject_str += 'd_pur_invoice_line' +'['+string(li_idx)+']'
istr_ds_4_posting.ds_master[li_idx] = create t_ds_db
istr_ds_4_posting.ds_master[li_idx].dataobject = 'd_invoice_pur_form'	
...
on c_pur_invoice.create
call super::create
end on

on c_pur_invoice.destroy
call super::destroy
end on
...
istr_actionpane[1].sa_sub_button[3]='b_view_detail_pur_inv;b_view_accounting;'
		...
		FOR li_idx = 1 to upperbound(istr_actionpane[]) 
			if istr_actionpane[li_idx].s_dwo_action = 'd_action_edit' then
				...
				if ldw_focus.dynamic f_get_ib_detail() then
					...
					if  ib_changed_dwo_4edit then
						istr_actionpane[li_idx].s_visible_buttons= 'b_add_multi;b_modify;b_filteron;b_refresh;b_delete;b_cancel;b_detail;b_allocate;b_copyt_multi;b_post;b_account_view;b_view_multi;b_send_2_approve;b_approve;b_reject;'
					else
						istr_actionpane[li_idx].s_visible_buttons='b_add_multi;b_modify;b_filteron;b_refresh;b_delete;b_cancel;b_detail;b_allocate;b_copyt_multi;b_post;b_account_view;b_view_multi;b_send_2_approve;b_approve;b_reject;'
					end if
				end if
...
event e_window_e_allocate;call super::e_window_e_allocate;iw_display.f_openchildwithparm('s_w_multi_ex','u_allocation_expense;0')
return 0
end event

event e_cbx_clicked;call super::e_cbx_clicked;// 
...
//if isvalid(ldw_view_tax) then
//	if rcbx_handling.checked then
//		is_filter_view_all = ldw_view_tax.describe("DataWindow.Table.Filter")
//		if is_filter_view_all = '?' then is_filter_view_all = ''
//		is_filter_view_all = lc_string.f_globalreplace( is_filter_view_all, "~~'", "'")
//		ldw_view_tax.setfilter( "")
//		ldw_view_tax.filter( )
//	else
//		ldw_view_tax.setfilter( is_filter_view_all)
//		ldw_view_tax.filter( )
//		is_filter_view_all = ''
//	end if
//end if
...
if ll_rtn > 0 then
	FOR ll_row = 1 to ll_rtn
		ls_t_table =  lds_matching.getitemstring( ll_row, 'T_REF_TABLE')
		if ls_t_table <> 'INVENTORY_LINE' then
			ls_f_doc_code = lds_matching.getitemstring( 1, 'f_doc_code')
			ls_t_doc_code = lds_matching.getitemstring( 1, 't_doc_code')
			if isnull(ls_f_doc_code) then ls_f_doc_code = ''
			if isnull(ls_t_doc_code) then ls_t_doc_code = ''
			gf_messagebox('m.c_pur_invoice.e_window_e_preunpost.01','Thông báo','Phiếu: '+ ls_f_doc_code +' đã kết với phiếu: '+ ls_t_doc_code+ ' nên không được sửa ghi sổ' ,'stop','ok',1)
			destroy lds_matching
			return -1			
		end if
	NEXT
elseif ll_rtn = 0 then
//	ldb_po_doc_id = this.f_get_po_doc_id( "PUR_INVOICE_LINE", double(laa_data[1]))
//	ls_status = 'full_receive'
////	ldc_qty = this.f_get_remain_qty_matching(ldb_so_doc_id )
//	if ldc_qty > 0 then ls_status = 'partial_receive'
//	update document set status = :ls_status where id = :ldb_po_doc_id using lt_transaction;
//	if lt_transaction.sqlcode <> 0 then
//		gf_messagebox('m.c_pur_invoice.e_window_e_preunpost.02','Thông báo','Lỗi, Không cập nhật được trạng thái cho đơn đặt mua: @'+lt_transaction.sqlerrtext ,'stop','ok',1)
...
=============================================================================================*/
int				li_cnt, li_idx, li_found
double		ladb_related_doc_id[], ldb_doc_id
string			ls_doctype_related, ls_doc_code_related, ls_doctype, ls_doc_code, ls_status, ls_status_related, ls_qty_yn, ls_value_yn,ls_object_type
date			ld_bookeddate_related, ld_bookeddate
boolean		lb_GR_yn = false

t_dw_mpl			ldw_main,ldw_line
c_dwservice			lc_dwservice

ldw_main = this.iw_display.f_get_dwmain( )
ldw_line = iw_display.f_get_dw( 'd_pur_invoice_line_grid')

ldb_doc_id = ldw_main.getitemnumber(ldw_main.getrow( ), 'id')
lc_dwservice.f_get_qty_and_value_yn(ldb_doc_id, ls_qty_yn, ls_value_yn)
li_found = ldw_line.find( "object_type = 'ITEM' ",1, ldw_line.rowcount())
if li_found > 0 then ls_object_type = 'ITEM'


if ls_object_type = 'ITEM' then
	li_cnt = this.f_get_f_doc_id(ldb_doc_id,ladb_related_doc_id[] )  						////-- chứng từ liên quan FROM --//
	if li_cnt = 0 then li_cnt = this.f_get_t_doc_id(ldb_doc_id,ladb_related_doc_id[]) 	////-- chtừ liên quan TO --//
		...
		FOR li_idx = 1 to li_cnt
			...
			if ls_doctype_related = 'GOODS_RECEIPT' then
				...
				if pos(ls_status_related+';','new;') > 0 then
					gf_messagebox('m.c_pur_invoice.e_window_e_prepost.02','Thông báo','Phiếu nhập kho của hóa đơn chưa ghi sổ:@'+ls_doc_code_related,'exclamation','ok',1)
					return -1
				end if
				if ld_bookeddate_related > ld_bookeddate then
					gf_messagebox('m.c_pur_invoice.e_window_e_prepost.03','Thông báo','Hóa đơn không được phép ghi sổ trước ngày nhập kho, phiếu nhập kho :@'+ls_doc_code_related+'@-ngày:'+ string(ld_bookeddate_related, gs_w_date_format ),'exclamation','ok',1)
					return -1
				end if
			elseif ls_doctype_related = 'PO' then
				
			end if
		NEXT
	else 
		if ls_qty_yn = 'Y' and ls_value_yn = 'N' then
			gf_messagebox('m.c_pur_invoice.e_window_e_prepost.04','Thông báo','Hóa đơn không được phép ghi sổ số lượng mà không ghi sổ giá trị!' ,'exclamation','ok',1)
			return -1		
		end if
		if ls_qty_yn = 'N' and ls_value_yn = 'Y' then
			gf_messagebox('m.c_pur_invoice.e_window_e_prepost.05','Thông báo','Hóa đơn không kết với phiếu nhập kho, phải chọn ghi sổ số lượng!' ,'exclamation','ok',1)
			return -1		
		end if	
	end if
end if
...
b_obj_instantiate		lbo_ins
	...
	if ldb_extend_id > 0 then
		...
		//-- PO_LINE--//
		DELETE  pur_invoice_line where object_ref_id = :ldb_extend_id using it_transaction;
...
if vs_objectname_to = 'u_payment' or vs_objectname_to = 'u_bank_voucher_payment'  then
	ldw_main = iw_display.f_get_dwmain( )
	if ldw_main.getrow()> 0 then
		if ldw_main.getitemstring( ldw_main.getrow(), 'status') = 'new' then
			gf_messagebox('m.c_pur_invoice.e_w_e_copy_to.01','Thông báo','Hoá đơn chưa ghi sổ','exclamation','ok',1)
			return -1
		end if
	end if
end if
