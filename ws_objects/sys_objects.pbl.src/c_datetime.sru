$PBExportHeader$c_datetime.sru
forward
global type c_datetime from c_obj_service
end type
end forward
...
forward prototypes
public function date f_gregorian (long al_julian)
public function long f_days (long al_seconds)
public function long f_hours (long al_seconds)
public function long f_yearsafter (date ad_start, date ad_end)
public function long f_millisecsafter (time atm_start, time atm_end)
public function integer f_wait (datetime adtm_target)
public function integer f_wait (unsignedlong al_seconds)
public function boolean f_isvalid (datetime adtm_source)
public function date f_firstdayofmonth (date ad_source)
public function boolean f_isleapyear (date ad_source)
public function boolean f_isweekday (date ad_source)
public function boolean f_isweekend (date ad_source)
public function datetime f_relativedatetime (datetime adtm_start, long al_offset)
public function long f_secondsafter (datetime adtm_start, datetime adtm_end)
public function long f_monthsafter (date ad_start, date ad_end)
public function string f_monthname (integer ai_monthnumber)
public function string f_monthname (date ad_source)
public function boolean f_isvalid (time atm_source)
public function boolean f_isvalid (date ad_source)
public function date f_relativemonth (date ad_source, long al_month)
public function date f_relativeyear (date ad_source, long al_years)
public function long f_weeksafter (date ad_start, date ad_end)
public function long f_juliandaynumber (date ad_source)
public function long f_julian (date ad_source)
public function long f_weeknumber (date ad_source)
public function integer f_dayofweek (date ad_source)
public function date f_lastdayofmonth (date ad_source)
public function double f_hourafter (datetime vt_start_time, datetime vt_end_time)
public function date f_add_months (date vd_date, integer vi_number)
public function date f_add_date (date vd_date, integer vi_number)
public function string f_getsysdate (string vs_date_format)
public function datetime f_get_date (string vs_date, string vs_date_format)
public function datetime f_add_datetime (datetime vd_date, decimal vdc_number)
public function datetime f_getsysdate ()
public function integer f_minuteafter (datetime vt_start_time, datetime vt_end_time)
public function integer f_minuteafter (time vt_start_time, time vt_end_time)
public function boolean f_isvalid (string vs_date, string vs_format)
public function date f_firstdaymonthofyear (date ad_source)
public function date f_lastdaymonthofyear (date ad_source)
public function date f_firstdaymonthofquarter (date ad_source)
public function date f_lastdaymonthofquarter (date ad_source)
public function integer f_string2date (string vs_datestring, string vs_format, ref date rd_date)
public function string f_get_sessiondate_format ()
public function date f_day_of_period (integer vi_nday, date vd_pay_date, date vd_date)
public function integer f_get_number_of_day (datetime vdt_f_date, datetime vdt_t_date)
public function integer f_get_number_of_day (date vd_f_date, date vd_t_date)
public function date f_get_weekend (date vd_s_date)
public function integer f_get_day_in_week (date vd_date)
public function string f_get_sessiondate_format_ex (ref t_transaction rt_transaction)
public function datetime f_get_date_ex (string vs_date, string vs_date_format, ref t_transaction rt_transaction)
public function datetime f_getsysdate_ex (ref t_transaction rt_transaction)
public function string f_getsysdate_ex (string vs_date_format, ref t_transaction rt_transaction)
public function date f_add_months_ex (date vd_date, integer vi_number, ref t_transaction rt_transaction)
end prototypes

public function date f_gregorian (long al_julian);//////////////////////////////////////////////////////////////////////////////
//
//	Function:  		f_Gregorian
//
//	Access:  		public
//
//	Arguments:
...
If (f_IsLeapYear(Date(li_year, 1, 1))) Then
...
If IsNull(ad_start) or IsNull(ad_end) or &
	Not f_IsValid(ad_start) or Not f_IsValid(ad_end) Then
	long ll_null
	SetNull (ll_null)
	Return ll_null
End If
...
If Not f_IsValid(ldt_value) Then
	Return -1
End If
...
ldtm_target = f_RelativeDatetime(DateTime(Today(),Now()), al_seconds)

//Perform the actual wait.
li_ret = f_Wait(ldtm_target)
...
//Check for invalid date
If Not f_IsValid(ad_source) Then
	Return ad_source
End If
...
//Check parameters
If IsNull(ad_source) or Not f_IsValid(ad_source) Then
	boolean lb_null
	SetNull(lb_null)
	Return lb_null
End If
...
Return Not f_IsWeekday(ad_source)
end function

public function datetime f_relativedatetime (datetime adtm_start, long al_offset);//////////////////////////////////////////////////////////////////////////////
//
//	Function:  		f_RelativeDatetime
//
//	Access:  		public
//
//	Arguments:
...
//Check for invalid date
If Not f_IsValid(adtm_start) Then
	Return ldt_null
End If
...
If IsNull(adtm_start) or IsNull(adtm_end) or &
	Not f_IsValid(adtm_start) or Not f_IsValid(adtm_end) Then
	long ll_null
	SetNull(ll_null)
	Return ll_null
End If
...
Return f_MonthName( Month(ad_source) )
...
//			relying on the f_IsValid(date) function to find an invalid date.  
...
If li_day > ii_daysinmonth[li_month] Then
	If li_month = 2 and f_isleapyear(date(li_year, 01, 01)) Then
		li_day = 29
	Else
...
If li_day > ii_daysinmonth[li_month] Then
   If li_month = 2 and &
      f_isleapyear(date(li_year, 01, 01)) Then
         li_day = 29
   Else
         li_day = ii_daysinmonth[li_month]
   end If
...
li_weeknumber = f_WeeksAfter (ld_first_ofyear, ad_source)
...
If (f_DayOfWeek(ld_first_ofyear) + li_leftover_days) >= 8 then
	li_weeknumber ++
End If
...
/*--------------------------------------------------------------------
         Khởi tạo: Út
             Ngày : 17-07-2009
		----------------------------------------------------------		 
 Nội dung xử lý: trừ thời gian kiểu datetime, trả về số gìơ giưa hai điểm thời gian.
    Giá trị trả về: Sô thực.
	 				> 0  Nếu thời gian đầu nhỏ hơn thời gian cuối.
					= 0 Nếu thời gian đầu bằng thời gian cuối.
					< 0  Nếu thời gian đầu lơn hơn thời gian cuối.
					*Null nếu một trong số các biến truyền vào là null
--------------------------------------------------------------------
Sửa:none

--------------------------------------------------------------------*/
return f_minuteafter(vt_start_time, vt_end_time)/60
end function
...
/*--------------------------------------------------------------------
         Khởi tạo: Út
             Ngày : 29-07-2009
		----------------------------------------------------------		 
 Nội dung xử lý: Cộng trừ tháng.
    Giá trị trả về: kiểu ngày
	 		         Ngày đã tăng hoặc giảm
					*Null nếu một trong số các biến truyền vào là null
--------------------------------------------------------------------
Sửa:none
--------------------------------------------------------------------*/
date ld_date 
if isnull(vd_date)  or isnull(vi_number) then 
	setnull(ld_date)
else 
	select add_months(:vd_date, :vi_number) into :ld_date from dual;
end if 
return ld_date
end function
...
if isnull(vd_date)  or isnull(vi_number) then 
	...
	select :vd_date + :vi_number into :ld_date from dual;
end if 
return ld_date
end function

public function string f_getsysdate (string vs_date_format);string ls_sysdate
SELECT to_char(sysdate, :vs_date_format) into :ls_sysdate from dual; 
...
public function datetime f_get_date (string vs_date, string vs_date_format);datetime ld_sysdate
SELECT to_date(:vs_date, :vs_date_format) into :ld_sysdate from dual; 
if sqlca.sqlcode < 0 then 
	setnull(ld_sysdate)
end if
return ld_sysdate
...
/*--------------------------------------------------------------------
         Khởi tạo: Dan
             Ngày : 24-09-2009
		----------------------------------------------------------		 
 Nội dung xử lý: cong tru ngay.
    Giá trị trả về: kieu ngay gio
	 		        
--------------------------------------------------------------------*/
datetime ld_date
if isnull(vd_date)  or isnull(vdc_number) then 
	setnull(ld_date)
else 
	select :vd_date + :vdc_number into :ld_date from dual;
end if 
return ld_date
end function

public function datetime f_getsysdate ();datetime ld_sysdate
...
public function integer f_minuteafter (datetime vt_start_time, datetime vt_end_time);/*--------------------------------------------------------------------
         Khởi tạo: Út
             Ngày : 17-07-2009
		----------------------------------------------------------		 
 Nội dung xử lý: trừ thời gian kiểu datetime, trả về phút
...
--------------------------------------------------------------------*/
int ll_minute_temp, ll_start, ll_end, ll_dateafter
date ld_start, ld_end
dec{2} ld_return 
if isnull(vt_start_time) or isnull(vt_start_time) then
	setnull(ll_minute_temp) 
	return ll_minute_temp
end if 
//get day number
ll_dateafter =  DaysAfter(date(vt_start_time), date(vt_end_time)) 
// get munite start
ll_minute_temp =  Minute(time(vt_start_time))
ll_start = ll_minute_temp
ll_minute_temp = hour(time(vt_start_time))*60
ll_start = ll_start + ll_minute_temp
// get munite end
ll_minute_temp =  Minute(time(vt_end_time))
ll_end = ll_minute_temp
ll_minute_temp = hour(time(vt_end_time))*60
ll_end = ll_end + ll_minute_temp
return ll_dateafter*24*60 + (ll_end - ll_start)
end function
...
--------------------------------------------------------------------*/
return secondsafter(vt_start_time,vt_end_time )/60
end function

public function boolean f_isvalid (string vs_date, string vs_format);date		ld_date

select to_date(:vs_date,:vs_format) into :ld_date from dual using SQLCA;
if SQLCA.sqlcode = 0 then
...
// Date (Year, Month, Day)
Return Date (Year(ad_source), 01, 01)
end function

public function date f_lastdaymonthofyear (date ad_source);///////////////////////////////
//
//Hàm lấy ngày cuối năm
//
///////////////////////////////
...
// Date (Year, Month, Day)
Return Date (Year(ad_source), 12, 31)
end function

public function date f_firstdaymonthofquarter (date ad_source);///////////////////////////////
//
//Hàm lấy ngày đầu quý
//
///////////////////////////////
...
If Not f_IsValid(ad_source) Then
	Return ad_source
End If
if month(ad_source) > 0 and month(ad_source) <=3 then
	li_month = 01
elseif month(ad_source) > 3 and month(ad_source) <=6 then
	li_month = 04
elseif month(ad_source) >= 7 and month(ad_source) <=9 then
	li_month = 07
elseif month(ad_source) >= 10 then
	li_month = 10
end if
// Date (Year, Month, Day)
Return Date (Year(ad_source), li_month, 01)
end function

public function date f_lastdaymonthofquarter (date ad_source);///////////////////////////////
//
//Hàm lấy ngày cuối quý
//
///////////////////////////////
...
// Date (Year, Month, Day)
Return Date (Year(ad_source), li_month, li_day)
end function

public function integer f_string2date (string vs_datestring, string vs_format, ref date rd_date);

select to_date(:vs_datestring, :vs_format) into :rd_date  from dual using sqlca;
...
public function string f_get_sessiondate_format ();/**************************
lấy định dạng ngày tháng của phiên
kết nối
**************************/

string ls_sessiondate_format

SELECT value into :ls_sessiondate_format
FROM nls_session_parameters
WHERE parameter='NLS_DATE_FORMAT' using SQLCA;
if SQLCA.sqlcode <> 0 then
	setnull(ls_sessiondate_format)
	return ls_sessiondate_format
else
	return ls_sessiondate_format
end if
...
public function date f_day_of_period (integer vi_nday, date vd_pay_date, date vd_date);/******************************
Lấy ngày thanh toán theo chu kỳ
vi_nday: chu kỳ thanh toán
vd_pay_date: ngày thanh toán (cài đặt)
vd_date: ngày ghi sổ
*******************************/
date	ld_rt_date

ld_rt_date = vd_pay_date

Do While ld_rt_date < vd_date
	ld_rt_date = this.f_add_date( ld_rt_date, vi_nday)
Loop

return ld_rt_date
end function

public function integer f_get_number_of_day (datetime vdt_f_date, datetime vdt_t_date);int		li_days

select trunc(:vdt_t_date) - trunc(:vdt_f_date) into :li_days from dual using SQLCA;

return li_days
end function

public function integer f_get_number_of_day (date vd_f_date, date vd_t_date);int		li_days

select :vd_t_date - :vd_f_date into :li_days from dual using SQLCA;

return li_days
end function

public function date f_get_weekend (date vd_s_date);int		li_day_in_week
date	ld_wk_date

select :vd_s_date + (7 - to_char(:vd_s_date,'D')) + 1 into :ld_wk_date from dual using SQLCA;
return ld_wk_date
end function

public function integer f_get_day_in_week (date vd_date);int		li_rtn

select to_char(:vd_date,'D') into :li_rtn from dual using SQLCA;
...
SELECT value into :ls_sessiondate_format
FROM nls_session_parameters
WHERE parameter='NLS_DATE_FORMAT' using rt_transaction;
if SQLCA.sqlcode <> 0 then
	setnull(ls_sessiondate_format)
	return ls_sessiondate_format
else
...
public function datetime f_get_date_ex (string vs_date, string vs_date_format, ref t_transaction rt_transaction);datetime ld_sysdate
SELECT to_date(:vs_date, :vs_date_format) into :ld_sysdate from dual using rt_transaction; 
...
public function datetime f_getsysdate_ex (ref t_transaction rt_transaction);datetime ld_sysdate
select sysdate into :ld_sysdate from dual using rt_transaction;
return ld_sysdate

end function

public function string f_getsysdate_ex (string vs_date_format, ref t_transaction rt_transaction);string ls_sysdate
SELECT to_char(sysdate, :vs_date_format) into :ls_sysdate from dual using rt_transaction; 
if sqlca.sqlcode < 0 then 
	ls_sysdate = '-1'
end if
return ls_sysdate
...
if isnull(vd_date)  or isnull(vi_number) then 
	...
	select add_months(:vd_date, :vi_number) into :ld_date from dual using rt_transaction;
end if 
return ld_date
end function

on c_datetime.create
call super::create
end on

on c_datetime.destroy
call super::destroy
end on
