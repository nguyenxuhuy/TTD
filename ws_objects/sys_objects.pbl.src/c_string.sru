$PBExportHeader$c_string.sru
forward
global type c_string from nonvisualobject
end type
end forward

global type c_string from nonvisualobject autoinstantiate
end type
...
//if ls_err = '' then
	...
	if pos(vs_data,'+') + pos(vs_data,'-')+ pos(vs_data,'*')+ pos(vs_data,'/') > 1 then
		ls_data = this.f_removewhitespace( vs_data)
		...
		if Match(mid(ls_data,li_pos1 -1,1), "[0-9]") and Match(mid(ls_data,li_pos1+1,1), "[0-9]") then
			return true
		else
			return false
		end if
		...
		if left(vs_data ,1) = '(' and right(vs_data,1) = ')' and (pos(vs_data,';') > 0 or pos(vs_data,',') > 0 )then			
			...
			if Match(vs_data, "[A-Za-z]") or Match(vs_data, "[<>=\$!]")  then
				return false
			else
				return true
			end if
