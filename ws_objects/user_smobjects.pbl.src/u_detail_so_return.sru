$PBExportHeader$u_detail_so_return.sru
forward
global type u_detail_so_return from b_detail
end type
end forward
...
global u_detail_so_return u_detail_so_return
...
on u_detail_so_return.create
call super::create
end on

on u_detail_so_return.destroy
call super::destroy
end on
...
//-- update TAX_LINE --//
if vs_colname = 'tax_pct' or vs_colname = 'tax_correction' then
	connect using it_transaction;
	ldb_so_line = rpo_dw.getitemnumber( vl_currentrow, 'id' )
	if isnull(vs_editdata) then vs_editdata = '0'
	if vs_colname = 'tax_pct' then
		ldc_tax_correction = rpo_dw.getitemnumber( vl_currentrow, 'tax_correction' )
		ldc_tax_pct = dec(vs_editdata)
		if isnull(ldc_tax_correction) then ldc_tax_correction = 0
		ls_dataStr = ' set tax_pct = '+vs_editdata
		ls_dataStr += ' , tax_correction = '+ string(ldc_tax_correction)
	else		
		ldc_tax_correction = dec(vs_editdata)
		ldc_tax_pct= rpo_dw.getitemnumber( vl_currentrow, 'tax_pct' )		
		if isnull(ldc_tax_pct) then ldc_tax_pct = 0
		ls_dataStr = ' set tax_pct = '+ string(ldc_tax_pct)
		ls_dataStr += ' , tax_correction = '+ vs_editdata
	end if
	...
	if li_tax_line_cnt > 0 then			
		...
		ll_tax_line_id = lb_obj.f_create_id_ex( it_transaction)
		ll_vat_id = 24262712
					
		ls_sql = "Insert into tax_line (id,company_id,branch_id,object_ref_id,object_ref_table, "+&
					" created_by,created_date,last_mdf_by,last_mdf_date,"+&
					"tax_id, tax_pct, tax_correction, trans_currency, exchange_rate)" +&
					" VALUES ( "+string(ll_tax_line_id)+ ","+string(gi_user_comp_id  )+ ","+string(gdb_branch  )+ ","+string(ldb_so_line ) +", 'SO_LINE', "+&
					string(gi_userid) + ", sysdate, "+string(gi_userid) + ", sysdate, "+string(ll_vat_id) +","+ string(ldc_tax_pct)+","+string(ldc_tax_correction)+","+ & 
