$PBExportHeader$ur_so_detail.sru
forward
global type ur_so_detail from b_report
end type
end forward

global type ur_so_detail from b_report
end type
global ur_so_detail ur_so_detail
...
iastr_dwo_tabpage[1].str_dwo[1].s_dwo_default = 'dr_so_detail_form'
iastr_dwo_tabpage[1].str_dwo[1].s_dwo_form = 'dr_so_detail_form'
...
iastr_dwo_tabpage[1].str_dwo[1].s_description ='Báo cáo danh sách đơn hàng'
...
istr_dwo[2].s_dwo_default = 'dp_so_detail_form'
istr_dwo[2].s_dwo_form = 'dp_so_detail_form'
...
istr_dwo_related[1].s_main_obj_dwo = 'dr_so_detail_form'		//datawindow quan hệ main
...
on ur_so_detail.create
call super::create
end on

on ur_so_detail.destroy
call super::destroy
end on
...
is_object_title = 'Báo cáo danh sách đơn hàng'

istr_actionpane[1].s_description = is_object_title

istr_report.s_dwo_report = 'dr_so_detail_form'
istr_report.b_multi_thread = false

end event

event e_window_e_postopen;call super::e_window_e_postopen;t_dw_mpl	ldw_parm
string 		ls_status,ls_doc_type,ls_invoice_request
...
ls_invoice_request = ldw_parm.getitemstring(ldw_parm.getrow(), 'a_invoive_request')
...
if isnull(ls_invoice_request) then 
	ls_invoice_request = '%'
	ldw_parm.setitem(ldw_parm.getrow(), 'a_invoive_request', ls_invoice_request)
end if
