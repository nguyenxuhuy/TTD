$PBExportHeader$ur_balance_sheet_t_tw.sru
forward
global type ur_balance_sheet_t_tw from b_report
end type
end forward

global type ur_balance_sheet_t_tw from b_report
end type
global ur_balance_sheet_t_tw ur_balance_sheet_t_tw
...
iastr_dwo_tabpage[1].str_dwo[1].s_dwo_default = 'dr_balance_sheet_t_tw_form'
iastr_dwo_tabpage[1].str_dwo[1].s_dwo_form = 'dr_balance_sheet_t_tw_form'
...
iastr_dwo_tabpage[1].str_dwo[1].s_description ='Bảng cân đối kế toán - so sánh'
...
istr_dwo[2].s_dwo_default = 'dp_balance_sheet_t_tw_form'
istr_dwo[2].s_dwo_form = 'dp_balance_sheet_t_tw_form'
...
on ur_balance_sheet_t_tw.create
call super::create
end on

on ur_balance_sheet_t_tw.destroy
call super::destroy
end on
...
is_object_title = 'Bảng cân đối kế toán - so sánh'

istr_actionpane[1].s_description = is_object_title

istr_report.s_dwo_report = 'dr_balance_sheet_t_tw_form'
istr_report.s_pro_name = 'pkg_balance_sheet_t_tw.pro_balance_sheet'
istr_report.sa_parm_name[1] = 'p_period_date_1'
istr_report.sa_parm_name[2] = 'p_period_date_2'
...
event e_dw_e_itemchanged;call super::e_dw_e_itemchanged;string	ls_day, ls_month, ls_year
int		li_year,li_day, li_month
date	ld_year
if AncestorReturnValue = 1 then return 1

if mid(rpo_dw.dataobject,1,len(rpo_dw.dataobject)-5) = 'dp_balance_sheet_t_tw' then
	if vs_colname = 'p_period_date_1' then
		// Tháng này năm trước
		...
		rpo_dw.setitem(vl_currentrow,'p_period_date_2',ld_year)
	end if
end if
