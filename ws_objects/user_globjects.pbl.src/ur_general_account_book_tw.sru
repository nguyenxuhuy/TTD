$PBExportHeader$ur_general_account_book_tw.sru
forward
global type ur_general_account_book_tw from b_report
end type
end forward

global type ur_general_account_book_tw from b_report
end type
global ur_general_account_book_tw ur_general_account_book_tw
...
iastr_dwo_tabpage[1].str_dwo[1].s_dwo_default = 'dr_general_account_book_tw_form'
iastr_dwo_tabpage[1].str_dwo[1].s_dwo_form = 'dr_general_account_book_tw_form'
...
on ur_general_account_book_tw.create
call super::create
end on

on ur_general_account_book_tw.destroy
call super::destroy
end on
...
istr_report.s_dwo_report = 'dr_general_account_book_tw_form'
...
ls_accept = ldw_parm.getitemstring( ldw_parm.getrow(), 'a_not_accept')
if isnull(ls_trans) then ldw_parm.setitem( ldw_parm.getrow(), 'a_trans_date','Y')
if isnull(ls_trans) then ldw_parm.setitem( ldw_parm.getrow(), 'a_not_accept','%')
return 0
end event
