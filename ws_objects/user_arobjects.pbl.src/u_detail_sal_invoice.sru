$PBExportHeader$u_detail_sal_invoice.sru
forward
global type u_detail_sal_invoice from b_detail
end type
end forward
...
global u_detail_sal_invoice u_detail_sal_invoice

type variables
double		idb_obj_ref_id, idb_dr_cr_object, idb_loc_id
...
public subroutine f_set_dwo_window ();istr_dwo[1].s_dwo_default =  'd_sal_invoice_line_grid'
istr_dwo[1].s_dwo_form = ''
istr_dwo[1].s_dwo_grid = 'd_sal_invoice_line_grid'
...
istr_dwo[1].s_description = 'Chi tiết hoá đơn bán'
end subroutine
...
istr_dwo_related[1].s_main_obj_dwo = 'd_sal_invoice_line_grid'		//datawindow quan hệ main
...
on u_detail_sal_invoice.create
call super::create
end on

on u_detail_sal_invoice.destroy
call super::destroy
end on
...
choose case vs_colname
	case 'amount' ,'qty', 'price','tax_pct','disc_pct','disc_amount'
		if vs_colname = 'amount' then
			//-- cập nhật các cột liên quan --//
			ls_upd_colname = 'price;base_amount;amount_ex_tax;base_amount_ex_tax;tax_amt;tax_correction;'
			ldw_handle = rpo_dw
		...
		elseif vs_colname =  'qty' or vs_colname =  'price' then
			...
			ls_upd_colname = 'amount;base_amount;amount_ex_tax;base_amount_ex_tax;tax_amt;tax_correction;'
			ldw_handle = rpo_dw
			if lb_obj.f_update_line_itemchanged_ex( vs_colname, vs_editdata , ls_upd_colname,vl_currentrow , ldw_handle, it_transaction , istr_currency) = -1 then return 1								
		elseif vs_colname =  'tax_pct' then
			//-- cập nhật các cột liện quan --//
			ls_upd_colname = 'tax_amt;tax_correction;'
			ldw_handle = rpo_dw
			if lb_obj.f_update_line_itemchanged_ex( vs_colname, vs_editdata , ls_upd_colname,vl_currentrow , ldw_handle, it_transaction , istr_currency) = -1 then return 1	
		elseif vs_colname =  'disc_pct' then
			//-- cập nhật các cột liện quan --//
			ls_upd_colname = 'amount;base_amount;amount_ex_tax;base_amount_ex_tax;tax_amt;tax_correction;disc_amount;'
			ldw_handle = rpo_dw
			if lb_obj.f_update_line_itemchanged_ex( vs_colname, vs_editdata , ls_upd_colname,vl_currentrow , ldw_handle, it_transaction , istr_currency) = -1 then return 1	
		elseif vs_colname =  'disc_amount' then
			//-- cập nhật các cột liện quan --//
			ls_upd_colname = 'amount;base_amount;amount_ex_tax;base_amount_ex_tax;tax_amt;tax_correction;disc_pct;'
			ldw_handle = rpo_dw
			if lb_obj.f_update_line_itemchanged_ex( vs_colname, vs_editdata , ls_upd_colname,vl_currentrow , ldw_handle, it_transaction , istr_currency) = -1 then return 1				
		end if
...
rpo_dw.setitem(vl_currentrow, 'dr_cr_object' , idb_dr_cr_object)
rpo_dw.setitem(vl_currentrow, 'booked_slip_loc_id' , idb_loc_id)
...
if ldw_main.getrow( ) > 0 then
	...
	idb_dr_cr_object =  ldw_main.getitemnumber(ldw_main.getrow(),'dr_cr_object')
	idb_loc_id =  ldw_main.getitemnumber(ldw_main.getrow(),'loc_id')
...
event e_dw_e_postsave;call super::e_dw_e_postsave;double 		ldb_sal_inv_line, ldb_vat_id, ldb_tax_line_id
int				li_tax_line_cnt, li_row
dec			ldc_tax_correction, ldc_tax_pct, ldc_tax_amt
...
FOR li_row = 1 to rpo_dw.rowcount()
	...
	ldc_tax_amt=  rpo_dw.getitemnumber( li_row, 'tax_amt' )	
	...
	ls_dataStr += ' , tax_amt = '+ string(ldc_tax_amt)
	
	ldb_sal_inv_line = rpo_dw.getitemnumber( li_row, 'id' )
	select count(id) into :li_tax_line_cnt from tax_line where object_ref_id = :ldb_sal_inv_line using it_transaction;
	if li_tax_line_cnt = 1 then
		ls_sql = "Update tax_line " + ls_dataStr + " where object_ref_id ="+string(ldb_sal_inv_line)
		execute immediate :ls_sql using it_transaction;	
	elseif li_tax_line_cnt = 0 then
		ldb_tax_line_id = lb_obj.f_create_id_ex( it_transaction)
		ldb_vat_id = 24262712  //-- TAX object --//
					
		ls_sql = "Insert into tax_line (id,company_id,branch_id,object_ref_id,object_ref_table, doc_version, "+&
					" created_by,created_date,last_mdf_by,last_mdf_date,"+&
					"tax_id, tax_pct, tax_amt, tax_correction, trans_currency, exchange_rate)" +&
					" VALUES ( "+string(ldb_tax_line_id)+ ","+string(gi_user_comp_id  )+ ","+string(gdb_branch  )+ ","+string(ldb_sal_inv_line ) +", 'SAL_INVOICE_LINE',"+ string(idb_obj_ref_id )+","+ &
					string(gi_userid) + ", sysdate, "+string(gi_userid) + ", sysdate, "+string(ldb_vat_id) +","+ string(ldc_tax_pct)+"," +string(ldc_tax_amt) +","+string(ldc_tax_correction)+","+ & 
