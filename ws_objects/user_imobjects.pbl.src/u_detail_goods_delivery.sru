$PBExportHeader$u_detail_goods_delivery.sru
forward
global type u_detail_goods_delivery from b_detail
end type
end forward
...
global u_detail_goods_delivery u_detail_goods_delivery
...
on u_detail_goods_delivery.create
call super::create
end on

on u_detail_goods_delivery.destroy
call super::destroy
end on
...
b_obj_instantiate		lbo_ins
	...
	if li_cnt > 0 then
		...
		if rpo_dw.rowcount() > 1 then
			lbo_ins.f_upd_related_status_del(ldb_doc_id, 'new', 'INVENTORY_LINE', ldb_doc_id, ldb_ref_id, it_transaction)
		elseif rpo_dw.rowcount() = 1 then
			lbo_ins.f_upd_related_status_del(ldb_doc_id, 'none', 'INVENTORY_LINE', ldb_doc_id, ldb_ref_id, it_transaction)
		end if
		//-- xoá --//
