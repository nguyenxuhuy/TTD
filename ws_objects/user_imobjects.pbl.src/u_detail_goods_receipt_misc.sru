$PBExportHeader$u_detail_goods_receipt_misc.sru
forward
global type u_detail_goods_receipt_misc from b_detail
end type
end forward
...
global u_detail_goods_receipt_misc u_detail_goods_receipt_misc
...
on u_detail_goods_receipt_misc.create
call super::create
end on

on u_detail_goods_receipt_misc.destroy
call super::destroy
end on
		...
		elseif vs_colname =  'qty' or vs_colname =  'price' then
			...
			if  vs_colname =  'qty' then
				...
				if ldc_origin_qty > ldb_qty then
					...
					lb_obj.f_upd_related_status(ldb_doc_id, 'partial_receive', it_transaction)
				else
					//-- kiểm mach remain --//
					...
					if ldc_match_remain < 0 then
						gf_messagebox('m.u_detail_goods_receipt.e_dw_e_itemchanged.01','Thông báo','Số lượng nhập kho VƯỢT quá đơn hàng','exclamation','ok',1)
						return -1
					end if
				end if
				//-- update match qty --//
...
b_obj_instantiate		lbo_ins
	...
	if li_cnt > 0 then
		...
		using  it_transaction;
		if li_cnt > 0 then
			lbo_ins.f_upd_related_status( ldb_doc_id, 'partial_receive', it_transaction)
		else			
