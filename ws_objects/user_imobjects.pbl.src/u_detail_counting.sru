$PBExportHeader$u_detail_counting.sru
forward
global type u_detail_counting from b_detail
end type
end forward
...
global u_detail_counting u_detail_counting
...
public subroutine f_set_dwo_window ();istr_dwo[1].s_dwo_default =  'd_inventory_line_counting_grid'
istr_dwo[1].s_dwo_form = ''
istr_dwo[1].s_dwo_grid = 'd_inventory_line_counting_grid'
...
istr_dwo[1].s_description = 'Chi tiết kiểm kê'
end subroutine
...
on u_detail_counting.create
call super::create
end on

on u_detail_counting.destroy
call super::destroy
end on
...
event e_dw_e_itemchanged;call super::e_dw_e_itemchanged;double			ldb_base_round_id
decimal			ldb_base_amount, ldb_adj_qty, ldb_doc_qty, ldb_price



if vs_colname = 'count_qty' then 
	ldb_doc_qty = rpo_dw.getitemnumber(vl_currentrow, 'doc_qty')
	
	ldb_price  = rpo_dw.getitemnumber(vl_currentrow, 'price' )
	
	if isnull(ldb_doc_qty) then ldb_doc_qty = 0
	ldb_adj_qty = double(vs_editdata) - ldb_doc_qty
	rpo_dw.setitem(vl_currentrow, 'qty', ldb_adj_qty )
	if ldb_adj_qty > 0 or ldb_adj_qty < 0 then	
		connect using it_transaction;
		ldb_price  = rpo_dw.getitemnumber(vl_currentrow, 'price' )
		ldb_base_round_id = ic_unit_instance.f_get_round_id_ex( 0, 0, 'bc', it_transaction )
		ldb_base_amount = this.ic_unit_instance.f_round(it_transaction , ldb_base_round_id, ldb_price*ldb_adj_qty)
		disconnect using it_transaction;
		rpo_dw.setitem(vl_currentrow, 'amount', ldb_base_amount )
//		rpo_dw.setitem(vl_currentrow, 'base_amount', ldb_base_amount )
	else
		setnull(ldb_base_amount)
		rpo_dw.setitem(vl_currentrow, 'amount', ldb_base_amount )
//		rpo_dw.setitem(vl_currentrow, 'base_amount', ldb_base_amount )
	end if
	rpo_dw.dynamic f_set_editable_column('price', true)
	rpo_dw.dynamic f_set_column_color('price', true)
elseif vs_colname = 'price' then 
	connect using it_transaction;
	ldb_base_round_id = ic_unit_instance.f_get_round_id_ex( 0, 0, 'bc', it_transaction )
	ldb_adj_qty = rpo_dw.getitemnumber(vl_currentrow, 'qty' )
	if ldb_adj_qty > 0 or ldb_adj_qty < 0 then
		ldb_base_amount = this.ic_unit_instance.f_round( it_transaction, ldb_base_round_id, ldb_adj_qty*double(vs_editdata))
		rpo_dw.setitem(vl_currentrow, 'amount', ldb_base_amount )
//		rpo_dw.setitem(vl_currentrow, 'base_amount', ldb_base_amount )
	else
		setnull(ldb_base_amount)
