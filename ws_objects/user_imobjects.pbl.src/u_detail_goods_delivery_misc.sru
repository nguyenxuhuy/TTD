$PBExportHeader$u_detail_goods_delivery_misc.sru
forward
global type u_detail_goods_delivery_misc from b_detail
end type
end forward
...
global u_detail_goods_delivery_misc u_detail_goods_delivery_misc
...
public subroutine f_set_dwo_window ();istr_dwo[1].s_dwo_default =  'd_inventory_line_delivery_grid'
istr_dwo[1].s_dwo_form = ''
istr_dwo[1].s_dwo_grid = 'd_inventory_line_delivery_grid'
...
istr_dwo[1].s_description = 'Chi tiết xuất kho'
end subroutine
...
on u_detail_goods_delivery_misc.create
call super::create
end on

on u_detail_goods_delivery_misc.destroy
call super::destroy
end on
...
event e_dw_e_itemchanged;call super::e_dw_e_itemchanged;double			ldb_item_id, ldb_spec_id, ldb_line_id, ll_lot_line_id, ldb_object_ref_id, ll_item_lot_id, ldb_doc_id
decimal			ldb_qty, ldc_adj_qty, ldc_origin_qty, ldc_match_remain
string				ls_upd_colname, ls_sql , ls_dataStr ,ls_manufacture_date, ls_expired_date, ls_lot_no, ls_t_ref_table
...
//			if lb_obj.f_update_line_itemchanged_ex( vs_colname, vs_editdata , ls_upd_colname,vl_currentrow , ldw_handle, it_transaction , istr_currency) = -1 then return 1	
		elseif vs_colname =  'qty' or vs_colname =  'price' then
			...
			ls_upd_colname = 'amount;base_amount;base_amount_ex_tax;'
			ldw_handle = rpo_dw
			if lb_obj.f_update_line_itemchanged_ex( vs_colname, vs_editdata , ls_upd_colname,vl_currentrow , ldw_handle, it_transaction , istr_currency) = -1 then return 1								
			//-- check matching --//
			if  vs_colname =  'qty' then
				ldb_line_id =  rpo_dw.getitemnumber( vl_currentrow,'id')
				ls_t_ref_table = rpo_dw.describe("datawindow.Table.updateTable")
				ldb_qty = dec(vs_editdata)
				ldc_origin_qty = rpo_dw.getitemnumber( vl_currentrow,'qty', primary!, true)
				connect using it_transaction;
				if ldc_origin_qty > ldb_qty then
					//-- update status --//					
					select id into :ldb_doc_id
					from document where version_id = :idb_obj_ref_id
					using it_transaction;
					lb_obj.f_upd_related_status(ldb_doc_id, 'partial_issue', it_transaction)
				else
					//-- kiểm mach remain --//
					ldc_adj_qty = ldb_qty - ldc_origin_qty
					ldc_match_remain = lb_obj.f_get_match_remain( ldb_line_id, ls_t_ref_table, ldc_adj_qty, 'qty', it_transaction)
					//--update--//
					if ldc_match_remain < 0 then
						gf_messagebox('m.u_detail_goods_delivery.e_dw_e_itemchanged.01','Thông báo','Số lượng xuất kho VƯỢT quá đơn hàng','exclamation','ok',1)
						return -1
					end if
				end if
				//-- update match qty --//
				update matching
				set qty = :ldb_qty
				where t_ref_id = :ldb_line_id and upper(t_ref_table) = upper(:ls_t_ref_table)
				using it_transaction;
				commit using it_transaction;
				disconnect using it_transaction;				
			end if
...
b_obj_instantiate		lbo_ins
	...
	if li_cnt > 0 then
		...
		from matching 
		where t_doc_id = :ldb_doc_id
		and t_ref_id <> :ldb_ref_id
		using  it_transaction;
		if li_cnt > 0 then
			...
			from matching 
			where t_doc_id = :ldb_doc_id and rownum = 1
			using  it_transaction;
			
			ls_ref_table = rpo_dw.describe("datawindow.table.Updatetable")
			select count(t_doc_id) into :li_cnt
			from matching 
			where f_doc_id = :ldb_f_doc_id
			and t_doc_id <> :ldb_doc_id 
			and t_ref_table = :ls_ref_table
			using  it_transaction;
			if li_cnt = 0 then
