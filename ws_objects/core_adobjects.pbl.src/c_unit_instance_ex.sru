$PBExportHeader$c_unit_instance_ex.sru
$PBExportComments$c_unit_instance
forward
global type c_unit_instance_ex from s_object
end type
end forward

global type c_unit_instance_ex from s_object
end type
global c_unit_instance_ex c_unit_instance_ex
...
public function integer f_get_data_4_conversion_unit_ex (double vdb_unit_id, double vdb_item_id, double vdb_data, ref double rdba_data_conver[], ref t_transaction rt_transaction)
public function string f_get_object_code_ex (double vdb_id, ref t_transaction rt_transaction)
public function double f_get_unit_inventory_id_ex (double vdb_item_id, ref t_transaction rt_transaction)
public function double f_get_round_diff_ex (double vdb_item_id, ref t_transaction rt_transaction)
public function integer f_get_exchange_account_info_ex (double vdb_curr_id, ref double rdba_exchange_account[], ref string rs_exchange_rate_method, ref string rs_trans_text, ref t_transaction rt_transaction)
end prototypes
...
ldb_unit_inventory_id = this.f_get_unit_inventory_id_ex(vdb_item_id , rt_transaction)


if ldb_unit_inventory_id > 0 then
	ldb_round_id = this.f_get_round_id_ex( ldb_unit_inventory_id , vdb_item_id, 'item', rt_transaction)
	if vdb_unit_id = ldb_unit_inventory_id then 
		rdba_data_conver[1] = vdb_data
		rdba_data_conver[2] = 0
		rdba_data_conver[3] =  1
	...
	select from_coefficient,to_coefficient,to_unit_id,round_diff into :ldb_from_coefficient,:ldb_to_coefficient,:ldb_to_unit_id,:ldb_diff
	from unit_conversion
	where item_id = :vdb_item_id and from_unit_id = :vdb_unit_id using rt_transaction;
	...
	elseif rt_transaction.sqlcode = 100 then
		...
		select count(from_unit_id) into :li_count
		from unit_conversion
		where item_id = :vdb_item_id and to_unit_id = :vdb_unit_id using rt_transaction;	
		if li_count > 1 then
			gf_messagebox('m.c_unit_instance.f_unit_conversion.03','Thông báo','Mã hàng: '+ls_item_code + ' có 2 quy đổi cùng 1 đơn vị','exclamation','ok',1)
			return -1
		end if
		...
		elseif rt_transaction.sqlcode = 100 then
			...
			select from_coefficient,to_coefficient,to_unit_id,round_diff into :ldb_from_coefficient,:ldb_to_coefficient,:ldb_to_unit_id,:ldb_diff
			from unit_conversion
			where from_unit_id = :vdb_unit_id and nvl(item_id,0) = 0 using rt_transaction;
			...
			if rt_transaction.sqlcode = 0 then
				...
				if ldb_unit_inventory_id = ldb_to_unit_id then
					...
					select from_coefficient,to_coefficient,round_diff into :ldb_from_coefficient_inven,:ldb_to_coefficient_inven,:ldb_diff
					from unit_conversion
					where from_unit_id = :ldb_unit_inventory_id and nvl(item_id,0) = 0 using rt_transaction;
					...
					if rt_transaction.sqlcode = 0 then
						...
						return 1
					elseif rt_transaction.sqlcode = 100 then
						ls_unit_code = this.f_get_object_code_ex(ldb_unit_inventory_id , rt_transaction)
						gf_messagebox('m.c_unit_instance.f_unit_conversion.02','Thông báo','Chưa cài đặt qui đổi cho đơn vị @'+ls_unit_code+' của mã hàng @: '+ls_item_code,'exclamation','ok',1)
						return -1
					else
						//--message lỗi : lỗi database--//
			...
			elseif rt_transaction.sqlcode = 100 then
				ls_unit_code = this.f_get_object_code_ex(vdb_unit_id , rt_transaction )
				gf_messagebox('m.c_unit_instance.f_unit_conversion.03','Thông báo','Chưa cài đặt qui đổi cho đơn vị @'+ls_unit_code+' của mã hàng @: '+ls_item_code,'exclamation','ok',1)
				return -1
			else
				//--message lỗi : lỗi database--//
...
into :rdba_exchange_account[1], :rdba_exchange_account[2], :rdba_exchange_account[3], :rdba_exchange_account[4], :rs_exchange_rate_method
from uom
where uom.id = :vdb_curr_id using rt_transaction;
...
on c_unit_instance_ex.create
call super::create
end on

on c_unit_instance_ex.destroy
call super::destroy
end on
