$PBExportHeader$ur_error_value.sru
forward
global type ur_error_value from b_report
end type
end forward

global type ur_error_value from b_report
end type
global ur_error_value ur_error_value
...
istr_dwo[2].s_dwo_default =  'dp_error_value_form'
istr_dwo[2].s_dwo_form = 'dp_error_value_form'
...
iastr_dwo_tabpage[1].str_dwo[1].s_dwo_default =  'dr_error_value_form'
iastr_dwo_tabpage[1].str_dwo[1].s_dwo_form = 'dr_error_value_form'
...
iastr_dwo_tabpage[1].str_dwo[1].s_description ='Báo cáo kiểm tra lỗi giá trị'
iastr_dwo_tabpage[1].str_dwo[1].s_gb_name = ''
end subroutine

on ur_error_value.create
call super::create
end on

on ur_error_value.destroy
call super::destroy
end on
...
is_object_title = 'Kiểm tra lỗi ghi sổ giá trị'

istr_actionpane[1].s_description = is_object_title

istr_report.s_dwo_report = 'dr_error_value'
istr_report.s_pro_name = 'pkg_check_val.pro_check_val'
...
event e_dw_e_preretrieve;call super::e_dw_e_preretrieve;string		ls_object_code
any 		laa_data[]

t_dw_mpl	ldw_parm

if rpo_dw.dataobject = 'dr_error_value_form' then
	ldw_parm = iw_display.f_get_dw( 'dp_error_value_form')
	if not isvalid(ldw_parm) then return 0
	ls_object_code = ldw_parm.getitemstring(1,'object_code')
	if not isnull(ls_object_code) and ls_object_code <> '' then
		laa_data[1] = ls_object_code
		rpo_dw.dynamic f_add_where( 'object_code', laa_data)
	end if
end if
return 0
end event
